...
// ─── Theme hook ──────────────────────────────────────────────
// Reads current theme from <body data-theme="..."> and re-renders when it flips.
const useTheme = () => {
...
// Swap asset path for dark mode. If a `-dark` variant exists for an image,
// switch to it when theme is dark. Extend DARK_MAP as more dark assets arrive.
const DARK_MAP = {
  "assets/app-01.png": "assets/app-01-dark.png",
  // "assets/app-02.png": "assets/app-02-dark.png",  // add when provided
};
const themedAsset = (src, theme) => (theme === "dark" && DARK_MAP[src]) ? DARK_MAP[src] : src;


// ─── Stripe checkout ─────────────────────────────────────────
const STRIPE_PK = "pk_live_51JSCe0Bv3rB1Oc3ZUd2Wy5j01B5L8mSPH3UqWGWoCUVl2Ih6hOahQ8QHIgDw6tQGtD60lfA1JskpiLtG0tiWgQmh00rmd4sHqP";
const PRICES = {
  personal: "price_1TPDdcBv3rB1Oc3Zukf4l8DU",
  pro: "price_1TPDo6Bv3rB1Oc3ZX3VgarLk",
};

let _stripeP;
const getStripe = () => {
  if (_stripeP) return _stripeP;
  _stripeP = new Promise((resolve, reject) => {
...
const checkout = async (tier, email) => {
  try {
    if (email) { try { localStorage.setItem("at_email", email); } catch {} }
    const stripe = await getStripe();
    const { error } = await stripe.redirectToCheckout({
      lineItems: [{ price: PRICES[tier], quantity: 1 }],
      mode: "subscription",
...
// ─── Nav ─────────────────────────────────────────────────────
// `page` is one of: "landing" | "pricing" | "cpas"
// Links resolve to in-page anchors when possible, otherwise cross-page.
const Nav = ({ onThemeToggle, theme, page = "landing" }) => {
  const href = (target) => {
    // target like "landing#features", "pricing", "cpas#how"
  ...
  <header className="nav">
    <div className="container nav-row">
      ...
      <div className="nav-actions">
        ...
        <a href="https://ap2.accountabletracker.com/" className="nav-signin">Sign in</a>
...
// ─── Email capture ───────────────────────────────────────────
const EmailCapture = ({ placeholder = "you@yourbusiness.com", cta = "Start free trial", tier = "personal", compact = false }) => {
  ...
  const submit = async (e) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) {
      setStatus("err"); setMsg("Please enter a valid email."); return;
    }
    setStatus("submitting"); setMsg("");
    try {
      const leads = JSON.parse(localStorage.getItem("at_leads") || "[]");
      leads.push({ email: val, ts: Date.now(), src: tier });
      localStorage.setItem("at_leads", JSON.stringify(leads));
      localStorage.setItem("at_email", val);
    } catch {}
    await checkout(tier, val);
    setStatus("idle");
  };
...
// ─── How It Works — interactive, real screenshots ────────────
const STEPS = [
  {
    n: 1, t: "Set up once",
    s: "Enter your business details — home office sq ft, phone %, entity type. We remember everything.",
    img: "assets/app-01.png",
    crop: { objectPosition: "center 10%" },
  },
  {
    n: 2, t: "Track monthly",
    s: "Log expenses as they happen. Home office, mileage, phone, out-of-pocket — all auto-calculated.",
    img: "assets/app-01.png",
    crop: { objectPosition: "center 55%" },
  },
  {
    n: 3, t: "Export & hand off",
    s: "One click generates an IRS-compliant PDF. Summary, totals, methods compared. Ready for your CPA.",
    img: "assets/app-02.png",
    crop: { objectPosition: "center 100%" },
  },
];
      ...
      <div className="container">
        ...
        <div className="how-wrap">
          <ol className="how-steps">
            {STEPS.map((s) => (
              <li key={s.n}
                  className={"how-step" + (step === s.n ? " on" : "")}
            ...
            <div className="how-browser">
              ...
              <div className="how-browser-body">
                {STEPS.map((s) => (
                  <img
                    key={s.n}
                    src={themedAsset(s.img, theme)}
                    alt={`Step ${s.n}: ${s.t}`}
                    className={"how-shot" + (step === s.n ? " on" : "")}
                    style={s.crop}
                  />
                ))}
              </div>
    ...
    <div className="container">
      ...
      <div className="price-grid">
        <article className="price-card">
          ...
          <button className="btn btn-ghost btn-lg" onClick={() => checkout("personal")}>Start 14-day trial</button>
        ...
        <article className="price-card featured">
          ...
          <button className="btn btn-primary btn-lg" onClick={() => checkout("pro")}>Start 14-day trial</button>
          <div className="price-foot">No credit card required</div>
        </article>
      </div>
    </div>
...
Object.assign(window, {
  ...
  Pricing, Testimonials, FAQ, FooterCTA, Footer, checkout, LogoMark,
  useTheme, themedAsset,
});
